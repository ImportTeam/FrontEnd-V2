...
function paymentCardToCardData(payment: PaymentCard): CardData {
  ...
  const bankName =
    (isUnknownType ? safeAlias : safeCardType)?.split(" ")[0] ||
    safeAlias ||
    safeCardType ||
    "Unknown";
...
export function CardsPageClient(props: CardsPageClientProps) {
  const { initialPayments, initialError } = props;
  ...
  const [cards, setCards] = useState<CardData[]>(() =>
    initialPayments.map((payment) => {
      try {
        ...
        log.error("Error converting payment card:", payment, error);
        return {
  ...
  const handleStartRegistration = async () => {
    try {
      ...
      const errorMsg =
        e instanceof Error ? e.message : "카드 등록을 시작할 수 없습니다.";
  ...
  const handleSetPrimary = async (id: string) => {
    try {
      ...
      const errorMsg =
        e instanceof Error ? e.message : "주 결제수단 설정에 실패했습니다.";
  ...
  return (
    <div className="space-y-8">
      ...
      {error ? (
        <div
          className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-300"
          role="alert"
        >
          {error}
        </div>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="secondary"
                        size="icon"
                        className="h-8 w-8 rounded-full bg-black/20 backdrop-blur-md hover:bg-black/40 text-white shadow-sm border-none"
                      >

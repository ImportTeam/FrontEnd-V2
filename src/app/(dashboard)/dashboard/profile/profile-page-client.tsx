...
export function ProfilePageClient({ initialProfile }: ProfilePageClientProps) {
  const log = logger.scope("PROFILE_PAGE_CLIENT");
  ...
  const [userProfile, setUserProfile] = useState<UserCurrentResponse | null>(initialProfile);
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-6 -mt-12">
            <div className="relative group">
              <Avatar className="h-28 w-28 border-4 border-white dark:border-zinc-900 shadow-xl">
                ...
                <AvatarFallback
                  className={`text-4xl ${avatarGradientClass} text-white font-bold`}
                >
                  {userInitial}
                </AvatarFallback>
              </Avatar>
              <button
                className="absolute bottom-0 right-0 p-2 rounded-full bg-white dark:bg-zinc-800 shadow-lg border-2 border-white dark:border-zinc-900 opacity-0 group-hover:opacity-100 transition-opacity"
                type="button"
